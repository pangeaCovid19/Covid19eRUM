...
che si riduce al modello esponenziale quando $M = 0$. Se ripetiamo l'analisi effettuata per il modello esponenziale, otteniamo una previsione di <span class="myclass">`r format(round(previsionequad$Attesi))`</span> casi totali, valore lontano <span class="myclass">`r round(deltaquad,1)`</span> deviazioni standard da quello reale. La probabilità di registrare un dato distante quanto o di più dalla previsione del dato reale, ammesso che il modello sia corretto, è pari a <span class="myclass">`r round(pvquad*100)`</span> per cento.


Tracciamo le previsioni dal 10 Marzo 2020 per il totale dei casi e per i deceduti. Per ogni data, sono riportati i valori registrati, quelli previsti dai due modelli addestrati con i dati fino al giorno prima e le deviazioni dato reale - previsione in termini di incertezza 
standard e P-value.
